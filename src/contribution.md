...
integral equation for each choice of the cutoff parameter $\Lambda = \beta\wmax$
...
## Early implementations (sparse-ir 1.x, SparseIR.jl 1.x)

The original Python implementation **sparse-ir 1.x** was mainly developed by
**Markus Wallerberger**. It provided a practical and widely used realization of
the intermediate representation for imaginary-time Green's functions.
...
## Unified low-level implementation (C++ and Rust)

Since around 2024, the focus has shifted towards **unifying the core
implementation** across languages via a common low-level library with a
C-compatible API.
...
The first attempt was a C++ library [`libsparseir`][libsparseir], developed
by **Satoshi Terasaki** and **Hiroshi Shinaoka**.

Subsequently, the project migrated to Rust as
[`sparse-ir-rs`][sparse-ir-rs], again led by **Satoshi Terasaki** and
**Hiroshi Shinaoka**. The new versions
...
ease of development, while still exposing a C-compatible API.
